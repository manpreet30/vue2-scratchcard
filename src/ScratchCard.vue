<!-- @format -->

<template>
  <!-- <img alt="Vue logo" src="./assets/logo.png" /> -->
  <div style="width: 400px; height: 200px">
    <canvas class="canvas-scratch-card" @mousedown="onMouseDown" @touchstart="onMouseDown" @mousemove="onMouseMove" @touchmove="onMouseMove" @mouseup="isDown = false" @touchend="isDown = false" ref="myCanvas"></canvas>
  </div>
</template>

<script>
import overlay from "@/assets/scratch-overlay.jpg";

export default {
  name: "ScratchCard",
  data: function () {
    return {
      cardObj: null,
      canvas: null,
      ctx: null,
      isDown: false,
      lastPoint: { x: 0, y: 0 },
      brushImg: new Image(),
      overlayImg: new Image(),
      width: 0,
      height: 0,
    };
  },
  mounted() {
    this.canvas = this.$refs.myCanvas;
    this.ctx = this.canvas.getContext("2d");

    this.width = this.canvas.width = this.canvas.offsetWidth;
    this.height = this.canvas.height = this.canvas.offsetHeight;

    // this.brushImg.src = brush;
    this.setBrushSrc();
    this.overlayImg.src = overlay;

    this.overlayImg.onload = () => {
      this.ctx.drawImage(this.overlayImg, 0, 0, this.width, this.height);
    };
  },
  methods: {
    getFilledInPixels() {
      const stride = 10;
      const pdata = this.ctx.getImageData(0, 0, this.width, this.height).data;
      const l = pdata.length;

      const total = l / stride;
      let count = 0;

      for (let i = (count = 0); i < l; i += stride) {
        if (parseInt(pdata[i]) === 0) count++;
      }

      return Math.round((count / total) * 100);
    },
    getMousePosition(e, canvas) {
      let offsetX = 0;
      let offsetY = 0;

      if (canvas.offsetParent !== undefined) {
        do {
          offsetX += canvas.offsetLeft;
          offsetY += canvas.offsetTop;
        } while ((canvas = canvas.offsetParent));
      }

      return {
        x: (e.pageX || (e.touches ? e.touches[0].clientX : 0)) - offsetX,
        y: (e.pageY || (e.touches ? e.touches[0].clientY : 0)) - offsetY,
      };
    },
    handlePercentage(filledInPixels) {
      if (filledInPixels && filledInPixels > 25) this.$emit("onScratchComplete");
    },
    onMouseDown(e) {
      e.preventDefault();
      e.stopPropagation();

      this.isDown = true;
      this.lastPoint = this.getMousePosition(e, this.canvas);
    },
    onMouseMove(e) {
      if (!this.isDown) return;

      e.preventDefault();
      e.stopPropagation();

      const currentPoint = this.getMousePosition(e, this.canvas);

      const dist = this.distanceBetween(this.lastPoint, currentPoint);
      const angle = this.angleBetween(this.lastPoint, currentPoint);

      for (let i = 0; i < dist; i++) {
        const x = this.lastPoint.x + Math.sin(angle) * i - 25;
        const y = this.lastPoint.y + Math.cos(angle) * i - 25;

        this.ctx.globalCompositeOperation = "destination-out";
        this.ctx.drawImage(this.brushImg, x, y);
      }

      this.lastPoint = currentPoint;
      this.handlePercentage(this.getFilledInPixels());
    },
    distanceBetween(p1, p2) {
      return Math.sqrt(Math.pow(p2.x - p1.x, 2) + Math.pow(p2.y - p1.y, 2));
    },
    angleBetween(p1, p2) {
      return Math.atan2(p2.x - p1.x, p2.y, p1.y);
    },
    setBrushSrc() {
      this.brushImg.src =
        "data:image/png;base64,";
      this.brushImg.onload = () => {
        this.brushImg.width = 10
        this.brushImg.height = 10
        console.log(this.brushImg.width);
      };
    },
  },
};
</script>

<style scoped>
.canvas-scratch-card {
  width: inherit;
  height: inherit;
  position: absolute;
  left: 0;
  top: 0;
}
</style>
